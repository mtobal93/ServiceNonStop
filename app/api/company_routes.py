...
from app.models import db, User, Review, Category, Company, Image
...
def get_all_companies():
    ...
    for company in companies:
        ...
        avg_stars = (
            ...
            .with_entities(func.avg(Review.stars))
            .scalar()
        )
...
def get_company_by_company_id(id):
    ...
    review_images = Image.query.filter(
        (Image.imageable_type == "review"), Image.imageable_id.in_(review_ids)
    ).all()
    ...
    company_dict["reviews"] = {"num_reviews": num_reviews, "avg_stars": None}
    ...
    company_images = Image.query.filter(
        (Image.imageable_type == "company"), Image.imageable_id == id
    ).all()
    company_image_urls = [
        {"id": image.id, "image_url": image.url, "uploaded_by_id": image.uploaded_by_id}
...
def get_user_companies():
    ...
    if companies is None:
        return {"message": "You have no companies"}, 404
...
def get_reviews_by_company_id(id):
    ...
    reviews = (
        Review.query.filter(Review.company_id == id)
    ...
    user_images = (
        Image.query.filter(Image.imageable_type == "user")
    ...
    all_company_review_images = Image.filter(Image.imageable_type != "user").all()
    ...
    for review in reviews:
        ...
        review_image_data = [
            {"id": image.id, "url": image.url, "uploaded_by_id": image.uploaded_by_id}
            for image in review_images
            if image.uploaded_by_id == review.user_id
        ]
...
def create_review(company_id):
    ...
    company_owner_id = get_company_by_company_id(company_id)["company"][0]["owner_id"]
    current_user_id = current_user.id

    forbidden = {"errors": {"message": "Forbidden"}}, 403
    ...
    existing_review = Review.query.filter(
        Review.user_id == current_user_id, Review.company_id == company_id
    ).first()
    if existing_review:
